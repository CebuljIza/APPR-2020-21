...
6. `izpusti` - podatki o izpustih ogljikovega dioksida
  - `Drzava` - ime države
  - `CO2_izpust_per_capita` - CO2 izpust v tonah na prebivalca
...
10. `nov.hdi` - tabela s šestimi indeksi in izračunanima starim in novim HDI-jem
  ...
  - `Ekoloski_indeks` - indeks, izračunan glede na izpust
  - `Indeks_COVID` - indeks, izračunan glede na število primerov po formuli, podani spodaj
...
### Novi indeks človekovega razvoja
Novi indeks človekovega razvoja poleg nivoja dohodka, izobrazbe in življenjske dobe upošteva še neenakost med prebivalci v teh treh komponentah, izpust ogljikovega dioksida v državi in število primerov novega koronavirusa na 1000 prebivalcev.

Tudi novo dodane spremenljivke vplivajo na človekov razvoj, vendar jih stari indeks ne upošteva.
...
Največ je indeks padel v prikazanih državah, seveda zaradi različnih razlogov. Med državami nas tudi nekatere presenetijo, na primer Hong Kong, Lihtenštajn in Češka. Za države bližnjega vzhoda, kot so Katar, Bahrajn, Palestina in Združeni Arabski Emirati pa predvidevamo, da je zaradi politike in religije v teh državah padec povzročil indeks neenakosti.
...
Poglejmo še rast novega indeksa v najbolj razvitih državah po starem indeksu ter v Sloveniji in v Združenih državah Amerike. Čeprav je razumljivo padel zaradi ekološkega odtisa in velikega števila primerov koronavirusa, padec nikjer ni bil dramatičen, s česar lahko sklepamo, da vse te države ohranjajo stabilne politike kar se tiče neenakosti, ekologije in ukrepov proti COVIDU-19. To so torej države, ki z ozirom na človekov razvoj v prihodnosti upoštevajo tudi druge spremenljivke poleg dohodka, zdravstva in izobrazbe.
...
Čeprav imajo prikazane države svoj indeks človekovega razvoja višji od vseh ostalih držav, so vseeno razlogi za padec v HDI-ju različni. Prve štiri države na grafu imajo zelo visoke nove indekse, z izjemo avstralskega ekološkega indeksa. Razlike med prebivalci so po indeksu neenakosti majhne, države pa se tudi trudijo ohranjati čim višji indeks COVID, vodi Avstralija, ki ima očitno daleč najmanj primerov na prebivalca.
...
**ZDA** pa imajo poleg indeksa COVID od vseh ostalih držav manjša tudi ostala dva indeksa. Razlika je zelo očitna in kaže na veliko večjo neenakost in ekološki odtis v ZDA kot pa v drugih podobno razvitih državah.